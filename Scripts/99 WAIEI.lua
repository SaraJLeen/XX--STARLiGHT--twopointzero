...
function yaGetRadarVal(song,player,prm,keymode,...)
	...
	if song then
		local r_mlr  =yaGetRD(player,'RadarCategory_Mines')/24+yaGetRD(player,'RadarCategory_Lifts')+(yaGetRD(player,'RadarCategory_Rolls')/1.5);
		...
		if prm=="Stream" then
			...
			if not keymode then
				if r_total>0 then
					ret=yaGetRD( player, 'RadarCategory_Stream')+(r_mlr/r_total)*0.3;
				else
					ret=0;
				end;
			else
		...
		elseif prm=="Voltage" then
			...
			over110=2;
			if not keymode then
				ret=yaGetRD( player, 'RadarCategory_Voltage');
			else
		...
		elseif prm=="Freeze" then
			...
			over110=2;
			if not keymode then
				ret=yaGetRD( player, 'RadarCategory_Freeze')+(0.9*yaGetRD( player, 'RadarCategory_Holds')/math.max(r_total, 1));
			else
		...
		elseif prm=="Chaos" then
			...
			over110=2;
			if not keymode then
				ret=yaGetRD( player, 'RadarCategory_Chaos')*1.05+yaGetRD(player,'RadarCategory_Mines')/1000;
			else
...
function GetScoreData(...)
	...
	elseif lprm=="combo" then	-- [ja] 最高フルコンボ判定 
		...
		elseif hicombo==9 then
			...
			--[[
			[ja] GOODフルコンの条件
			COMBO>=TAP&Notes and notE判定 and W5/Miss/NG/Hit 0
			--]]
			for i=1,#hiscore do
	...
	elseif lprm=="maxcombo" then	-- [ja] 最高MAXコンボ 
		...
		return hicombo;
	else	-- [ja] 最高DP/その時のスコア/hiscore型 （secondの場合は2位のデータ）
		local hidp = 0;
		local hi_i = 1;
		local hidp_2 = 0;
		local hi_i_2 = 1;
		for i=1,#hiscore do
            local check_hidp = hiscore[i]:GetPercentDP();
			if check_hidp > hidp then
                hidp_2 = hidp;
                hi_i_2 = hi_i;
				hidp = check_hidp;
				hi_i = i;
			end;
            if i ~= hi_i and check_hidp <= hidp and check_hidp > hidp_2 then
                hidp_2 = check_hidp;
                hi_i_2 = i;
            end;
		end;
		if lprm=="score" then
			return hiscore[second and hi_i_2 or hi_i]:GetScore();
		elseif lprm=="dp" then
			return second and hidp_2 or hidp;
		elseif lprm=="hiscore" then
			return hiscore[second and hi_i_2 or hi_i];
		else
			return second and hi_i_2 or hi_i;
		end;
	end;
end;
