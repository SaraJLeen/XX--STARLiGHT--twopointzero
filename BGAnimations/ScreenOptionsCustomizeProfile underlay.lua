...
menu_items[#menu_items+1]= {name= "avatimg_edit", item_type= "menu", get=function()
	...
	return ProfileImage and string.gsub(string.gsub(ProfileImage, "/Appearance/Avatars/",""), ".png","") or "default"
end,
screen_name= "ScreenAvatarImageSelection"}
menu_items[#menu_items+1]= {name= "exit", item_type= "exit"}

local menu_pos= 1
local menu_start= SCREEN_CENTER_Y-320
local menu_x= 100
	...
	if item.get then
		...
		if item.item_type == "list" then
			...
			value_args[#value_args+1]= Def.ActorMultiVertex{
				...
				PressLeftCommand= function(s) s:stoptweening():linear(.2):zoom(2):linear(.2):zoom(1),
			}
			value_args[#value_args+1]= Def.ActorMultiVertex{
				InitCommand= function(self)
					self:SetVertices{
				...
				PressRightCommand= functions(s) s:stoptweening():linear(.2):zoom(2):linear(.2):zoom(1),
			}
		end
