...
local maximum_array_size = math.floor( SCREEN_WIDTH / 100 )-( SCREEN_WIDTH > 1280 and 1 ) 
...
local spacingyscale = 5.7
local spacingxscale = 101
local isinit = false
local t = Def.ActorFrame{
    ChangeItemsMessageCommand=function(s,param)
        ...
        CImg:Load( img_cache[c][2] ):setsize(100,100)
...
local ImgGrid = Def.ActorFrame{
    ScrollGridMessageCommand=function(s,param)
        s:stoptweening():decelerate(0.2):y( -102.6*(param.tileoffset-5) )
    end,
}
if #img_cache > 0 then
    for i,v in pairs( img_cache ) do
        ...
        ImgGrid[#ImgGrid+1] = Def.ActorFrame{
            OnCommand=function(s)
                s:xy( 50+math.mod( i-1, maximum_array_size )*spacingxscale, SCREEN_CENTER_Y-300+( math.mod(i-1, maximum_array_size*yarrayamm)-math.mod(i-1, maximum_array_size) )*spacingyscale )
            end,
			Def.Quad{
				OnCommand=function(s)s:diffuse(color("#000000")):setsize(100,100):diffusealpha(0):addy(20):sleep(0.004*i):decelerate(0.1):addy(-20):diffusealpha(0.5) end,
				OffCommand=function(s) s:diffuse(color("#000000")):stoptweening():sleep( 0.004*i):accelerate(0.1):addy(20):diffusealpha(0) end,
			},
            Def.Sprite{
                Texture=v[2],
                OnCommand=function(s)s:setsize(100,100):diffusealpha(0):addy(20):sleep(0.004*i):decelerate(0.1):addy(-20):diffusealpha(1) end,
...
t[#t+1] = Def.Quad{
	OnCommand=function(s)
		s:xy( 20,135 ):halign(0):zoomto( 400, 100 ):diffuse( color("#00000080") )
		s:diffusealpha(0):linear(0.2):diffuse( color("#00000080") )
	end,
	OffCommand=function(s) s:linear(0.2):diffusealpha(0) end,
}
